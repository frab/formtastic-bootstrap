...
module ActionView
  module Helpers
    ...
    module TextFieldDateHelper
      ...
      def datetime_text_field(object_name, method, options = {}, html_options = {})
        InstanceTag.new(object_name, method, self, options.delete(:object)).to_datetime_text_field_tag(options, html_options)
      end
      ...
      def text_field_time(time = Time.zone.now, options = {}, html_options = {})
        time ||= Time.zone.now
        DateTimeSelector.new(time, options, html_options).text_field_time
      end

      def text_field_datetime(datetime = Time.zone.now, options = {}, html_options = {})
        datetime ||= Time.zone.now
        DateTimeSelector.new(datetime.in_time_zone, options, html_options).text_field_datetime
      end
    end
    ...
    class DateTimeSelector
      ...
      def text_field_month
        unless @options[:use_hidden] || @options[:discard_month]
          build_text(:month, month.nil? ? "" : month, :leading_zeros => false)
        end
      end
    ...
    class InstanceTag #:nodoc:
      ...
      def to_datetime_text_field_tag(options = {}, html_options = {})
       datetime_selector(options, html_options).text_field_datetime.html_safe
      end
    end
